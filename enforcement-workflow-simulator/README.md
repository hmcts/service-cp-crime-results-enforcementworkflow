...
- **Not part of the service artefact.** It builds and ships as its own boot jar. It is never
  ...
  [ADR-001](../docs/pipeline/adrs/001-enforcement-workflow-simulator-subproject.md).
- **Not deployable to a live environment**, by design — see "Guard layers" below.

## How to run it

```bash
SPRING_PROFILES_ACTIVE=enforcement-workflow-simulator ./gradlew :enforcement-workflow-simulator:bootRun
```

Listens on port `8091` (override with `SERVER_PORT`). The `enforcement-workflow-simulator` profile is the default
...
2. **A profile allow-list.** `application.yaml` only activates the simulator's beans under the
   `enforcement-workflow-simulator` Spring profile.
3. **A pre-bean fail-fast.** `LiveEnvironmentGuard` listens for
   ...
   `perf`, or `preprod` is an active profile, or if `enforcement-workflow-simulator` is not. A misdeployed pod
...
Three YAML catalogue resources under `src/main/resources/enforcement-workflow-simulator/catalogue/` encode the
...
1. `${ENFORCEMENT_WORKFLOW_SIMULATOR_SEED_DIR}/<caseUrn>.json` — an external directory, if the environment variable
   is set and the file exists.
2. `enforcement-workflow-simulator/seeds/<caseUrn>.json` on the classpath — bundled seeds, under
   `src/main/resources/enforcement-workflow-simulator/seeds/`.
...
1. Create `src/main/resources/enforcement-workflow-simulator/seeds/<caseUrn>.json` (or drop it in the directory
   named by `ENFORCEMENT_WORKFLOW_SIMULATOR_SEED_DIR` for a non-bundled environment).
...
`src/test/resources/enforcement-workflow-simulator/fixtures/<name>/` containing `request.json` and
...
1. Edit the relevant YAML file(s) under `src/main/resources/enforcement-workflow-simulator/catalogue/`. Add new
...
4. If the contract version changes, replace the file under `src/main/resources/openapi/` and
   ...
   generated.
5. Run `:enforcement-workflow-simulator:test` (below) — a broken catalogue fails fast at Spring context startup in
   every integration test, so you will not get a silent wrong answer.

## Running the tests and PMD

```bash
./gradlew :enforcement-workflow-simulator:test
```
...
(equally, `./gradlew :enforcement-workflow-simulator:pmdMain` — `gradle/pmd.gradle` matches either form). It is
...
```bash
./gradlew :enforcement-workflow-simulator:test :enforcement-workflow-simulator:pmdMain \
...
- **`SeedStore`'s path-traversal guard is looser than its name suggests.** The only guard against
  `../../application`-style escapes via `ENFORCEMENT_WORKFLOW_SIMULATOR_SEED_DIR` is the regex
