...
Add `:gob-simulator` as its own Gradle subproject with its own Spring Boot boot jar, declared in a
...
2. **`gradle/pmd.gradle`** — the opt-in `pmdMain` gate (only runs when explicitly requested by
   ...
   invoked as `pmdMain` from within the subproject or `:gob-simulator:pmdMain` from the root.
...
3. **A pre-bean fail-fast** (`LiveEnvironmentGuard`) that throws before any bean is created if a
   ...
   `gob-simulator` is not.

## Consequences

- The repo now has a `settings.gradle` it did not have before, and a second `build.gradle` under
  `gob-simulator/`. The template's single-module assumption no longer holds for this repo.
...
- CI and local tooling that assume a single build output (`build/libs/*.jar`) continue to work
  ...
  `gob-simulator/build/libs/*.jar` explicitly and does not exist yet (Helm/deployment for the
...
- PMD for the simulator must be invoked explicitly (`./gradlew pmdMain` or
  `:gob-simulator:pmdMain`) — it does not run as part of `build` for either project.
