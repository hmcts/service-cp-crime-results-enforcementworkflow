...
```
settings.gradle                     # NEW — rootProject.name + include ':gob-simulator'
...
└── src/
    ├── main/java/uk/gov/hmcts/cp/gobsimulator/
    │   ├── GobSimulatorApplication.java
    ...
    │   ├── gob-simulator/catalogue/{entity-names,field-paths,result-codes}.yaml
    │   └── gob-simulator/seeds/<caseUrn>.json
    └── test/java/uk/gov/hmcts/cp/gobsimulator/...
```

**Why root-as-service rather than `:app` + `:gob-simulator`:** splitting out an `:app` subproject
...
1. **Nothing packages it.** The root `Dockerfile` adds only the service boot jar. No workflow in
   ...
   `:gob-simulator` subproject is excluded from `publishing`.
2. **Allow-list.** `GobSimulatorApplication` refuses to start unless the `gob-simulator` Spring
...
Seeds are `gob-simulator/seeds/<caseUrn>.json` on the classpath, loaded at startup, with the
directory overridable via `GOB_SIMULATOR_SEED_DIR` for environment-specific data. Chosen over a
...
**Build**: `./gradlew :gob-simulator:test` plus the root build, both green before review.
...
| Implementation-plan HTML artifact — **mandatory before Stage 5** | `docs/pipeline/artifacts/002-gob-simulator-implementation-plan.html` |
| ADR-001 — simulator as a Gradle subproject in the service repo (template deviation) | `docs/pipeline/adrs/001-gob-simulator-subproject.md` |
| ADR-002 — v0.3.0 schema chosen over the CIMD-4372 examples | `docs/pipeline/adrs/002-v030-schema-over-jira-examples.md` |
| This spec | `docs/superpowers/specs/2026-09-17-gob-libra-simulator-design.md` |
